...
The installer for gws 0.9.1

.DESCRIPTION

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/googleworkspace/cli/releases/download/v0.9.1
...
$app_name = 'gws'
$app_version = '0.9.1'
if ($env:GWS_DOWNLOAD_URL) {
  $ArtifactDownloadUrls = @($env:GWS_DOWNLOAD_URL)
} elseif ($env:INSTALLER_DOWNLOAD_URL) {
  $ArtifactDownloadUrls = @($env:INSTALLER_DOWNLOAD_URL)
} elseif ($env:GWS_INSTALLER_GHE_BASE_URL) {
  $installer_base_url = $env:GWS_INSTALLER_GHE_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/googleworkspace/cli/releases/download/v0.9.1")
} elseif ($env:GWS_INSTALLER_GITHUB_BASE_URL) {
  $installer_base_url = $env:GWS_INSTALLER_GITHUB_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/googleworkspace/cli/releases/download/v0.9.1")
} else {
  $ArtifactDownloadUrls = @("https://github.com/googleworkspace/cli/releases/download/v0.9.1")
}

$auth_token = $env:GWS_GITHUB_TOKEN

$receipt = @"
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.31.0"},"source":{"app_name":"gws","name":"cli","owner":"googleworkspace","release_type":"github"},"version":"0.9.1"}
"@
if ($env:XDG_CONFIG_HOME) {
  $receipt_home = "${env:XDG_CONFIG_HOME}\gws"
} else {
  $receipt_home = "${env:LOCALAPPDATA}\gws"
}

if ($env:GWS_DISABLE_UPDATE) {
...
if ($NoModifyPath) {
    Write-Information "-NoModifyPath has been deprecated; please set GWS_NO_MODIFY_PATH=1 in the environment"
}

if ($env:GWS_NO_MODIFY_PATH) {
    $NoModifyPath = $true
}

$unmanaged_install = $env:GWS_UNMANAGED_INSTALL
...
function Install-Binary($install_args) {
  ...
  $platforms = @{
    "aarch64-pc-windows-msvc" = @{
      "artifact_name" = "gws-x86_64-pc-windows-msvc.zip"
      "bins" = @("gws.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
...
function Invoke-Installer($artifacts, $platforms) {
  ...
  if (($env:GWS_INSTALL_DIR)) {
    $force_install_dir = $env:GWS_INSTALL_DIR
  ...
  if ($install_updater) {
    ...
    [IO.File]::WriteAllLines("$receipt_home/gws-receipt.json", "$receipt", $Utf8NoBomEncoding)
  }
